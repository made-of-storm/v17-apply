...
### 4. Почта отказов (noreply@v17.vc)

Отправитель для заявителей — **только** `noreply@v17.vc`. Личный Gmail может
владеть скриптом и таблицей — это нормально: получатели **не видят** вашу личную
почту, если настроен алиас ниже. Без алиаса отказы **не отправляются** (раньше
уходили с личного ящика подписью «V17 Team» — так больше не делаем).

**Настройка алиаса в Gmail владельца скрипта** (один раз):

1. Войти в Gmail того Google-аккаунта, под которым развёрнут Apps Script
2. ⚙️ → **Все настройки** → **Аккаунты и импорт** → **Отправлять письма как** → **Добавить другой адрес**
3. Имя: `V17 Team`, адрес: `noreply@v17.vc`
4. Снять галочку «Обрабатывать как алиас» → **Следующий шаг**
5. SMTP: `smtp.gmail.com`, порт `587`, TLS; логин `noreply@v17.vc`, пароль от ящика
6. Подтвердить письмом из noreply@v17.vc (или кодом)
7. В Apps Script: **Развернуть** → **Управление развёртываниями** → ✏️ → **Новая версия** → Развернуть

**Тест:** в редакторе Apps Script выбрать `testDeclineMail`, вписать свой email в
`TEST_DECLINE_TO`, нажать **Выполнить**. В ответе должно быть
`usedFallback: false`, `from: noreply@v17.vc`.

**Антиспам (DNS у регистратора домена v17.vc):**
...
**Тестовые заявки (авто):** если в email `@example.com` / `e2e-test-…`, в названии
компании есть `test` / `e2e` / `тест`, или в `source` пометка `test` — заявка
пишется только на лист **Tests**, без Notion и Telegram. Реальные заявки не затрагиваются.

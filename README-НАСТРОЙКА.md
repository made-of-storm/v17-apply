...
- Notion → Telegram: свежий статус **Declined** (также Rejected / Отказ) раз в минуту
  догоняет чат. Если в таблице или в свойстве Email карточки есть адрес —
  уходит то же письмо, что и у кнопки «Отказ (стандарт)», кнопки под заявкой
  снимаются. Нет email (типичная CRM-карточка без заявки с формы) — в чат
  приходит пометка, письмо не шлётся. Исторические Declined не трогаем:
  первый прогон смотрит только последние ~3 часа.
  После обновления скрипта ещё раз выполнить `setupTelegramPolling`
  (или вручную поставить минутный триггер на `syncNotionToTelegram`).
...
- Автоматических писем при подаче нет: заявки из soft-зоны просто помечаются
  как отдельный пул (на форме им честно написано «cannot guarantee a response»).
  Отказ уходит вручную: кнопками из Telegram или сменой статуса на Declined
  в Notion (стандартный шаблон, только если есть email).
...
- Второй тест: новая заявка с настоящим email → в Notion статус **Declined** →
  в течение минуты в чате «Отказ в Notion», письмо-стандарт на тот же email.
  Не проверять на старых CRM-сделках без заявки с формы (у них часто нет email).
